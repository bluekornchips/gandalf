...
SHELL_MANAGER="$SCRIPT_DIR/shell-tests-manager.sh"
...
run_python_tests() {
    ...
    if ! python_tests_exist; then
        echo "Warning: Python tests not found at tests/python/" >&2
        return 0
    fi
...
count_python_tests() {
    ...
    local count
    count=$(python3 -m pytest tests/python/ --collect-only -q 2>/dev/null | grep -c "::" 2>/dev/null || echo "0")
    echo "$count"
}

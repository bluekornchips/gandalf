...
class TestProjectSecurityIntegration:
    ...
    def test_all_functions_use_security_validator(self):
        ...
        with patch(
            ...
            get_project_names(Path("/path/to/orthanc"))
            assert mock_sanitize.call_count == 2

            # Test get_sanitized_project_name
            get_sanitized_project_name(Path("/path/to/orthanc"))
            assert mock_sanitize.call_count == 3

            # Verify all calls were with the same project name

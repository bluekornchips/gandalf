...
from src.tool_calls.claude_code_query import (
    ...
    handle_search_claude_conversations,
)
...
class TestClaudeCodeQuery:
    ...
    def test_search_conversations_found(self):
        ...
        with patch.object(
            ...
            assert (
                results[0]["match_count"] == 2
            )  # Both messages contain "Python"
    ...
    def test_search_conversations_no_matches(self):
        ...
        self.create_test_session_file(
            self.project_sessions_dir, "js-session", messages
        )
    ...
    def test_extract_snippet(self):
        ...
        with patch.object(
            ...
            text = (
                "This is a long text with the word Python in the middle of it"
            )
...
class TestClaudeCodeQueryHandlers:
    ...
    def test_handle_query_claude_conversations_success(self, mock_query_class):
        ...
        result = handle_query_claude_conversations(
    ...
    @patch("src.tool_calls.claude_code_query.ClaudeCodeQuery")
    def test_handle_query_claude_conversations_markdown(
        ...
        assert len(result["content"]) == 1
        assert result["content"][0]["type"] == "text"
        assert result["content"][0]["text"] == "# Test Markdown"

    @patch("src.tool_calls.claude_code_query.ClaudeCodeQuery")
    def test_handle_search_claude_conversations_success(
        self, mock_query_class
    ):
        """Test successful conversation searching handler."""
        # Mock the query tool
        mock_query = Mock()
        mock_query_class.return_value = mock_query
        ...
        arguments = {"query": "test", "limit": 20, "include_content": False}
        result = handle_search_claude_conversations(
            arguments, self.project_root
        )
    ...
    def test_handle_search_claude_conversations_missing_query(self):
        """Test conversation searching handler with missing query."""
        arguments = {}
        result = handle_search_claude_conversations(
            arguments, self.project_root
        )
    ...
    @patch("src.tool_calls.claude_code_query.ClaudeCodeQuery")
    def test_handle_search_claude_conversations_with_content(
        self, mock_query_class
    ):
        """Test conversation searching handler with content inclusion."""
        # Mock the query tool
        mock_query = Mock()
        mock_query_class.return_value = mock_query
        mock_query.search_conversations.return_value = [
        ...
        arguments = {"query": "test", "include_content": True}
        result = handle_search_claude_conversations(
            arguments, self.project_root
        )
        ...
        data = json.loads(result["content"][0]["text"])
        assert (
            data["results"][0]["matches"][0]["full_content"]
            == "full message content"
        )

    @patch("src.tool_calls.claude_code_query.ClaudeCodeQuery")
    def test_handle_query_claude_conversations_exception(
        self, mock_query_class
    ):
    ...
    @patch("src.tool_calls.claude_code_query.ClaudeCodeQuery")
    def test_handle_search_claude_conversations_exception(
        self, mock_query_class
    ):

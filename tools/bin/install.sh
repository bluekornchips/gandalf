...
install_for_claude_code() {
	...
	if ! is_application_installed "claude"; then
		echo "Error: Claude Code not found. Please install Claude Code first." >&2
		return 1
	fi

	if ! command -v claude &>/dev/null; then
cat <<EOF
Warning: Claude CLI not available for MCP configuration
Claude Code detected but CLI not found. Skipping MCP setup
Note: Claude Code MCP may need manual configuration
EOF
		return 0
	fi

	# Try to install MCP configuration, but don't fail if it doesn't work
	...
	if claude mcp add "$server_name" "$gandalf_root/gandalf" -s user "run" \
		...
		echo "Warning: Claude Code MCP configuration failed, but continuing installation"

...
execute_conversation_tool() {
	...
	if [[ -z "$content" ]]; then
		echo "Empty content returned from $tool_name" >&2
		return 1
	fi
...
validate_conversation_response() {
	...
	# Clean the content and ensure it's properly formatted for jq
	local cleaned_content
	cleaned_content=$(echo "$content" | tr -d '\n' | sed 's/[[:space:]]\+/ /g')

	# Try to parse as JSON
	if echo "$cleaned_content" | jq . >/dev/null 2>&1; then
		# Check if this is an error response about no agentic tools being available
		if echo "$cleaned_content" | jq -e '.content[0].text' >/dev/null 2>&1; then
			local error_text
			error_text=$(echo "$cleaned_content" | jq -r '.content[0].text' 2>/dev/null)
			if [[ "$error_text" == *"No agentic tools available"* ]]; then
				return 0
			fi
		fi
		...
		for field in "${fields[@]}"; do
			field=$(echo "$field" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//') # trim whitespace
			if echo "$cleaned_content" | jq -e "has(\"$field\")" >/dev/null 2>&1; then
				has_field=true
				break
			fi
		done
		...
		if [[ "$has_field" == true ]]; then
			...
			# Check if this is still a valid response structure (like a structured error)
			if echo "$cleaned_content" | jq -e 'has("tool_results") or has("available_tools") or has("processing_time")' >/dev/null 2>&1; then
				return 0
			fi

			echo "None of expected fields ($expected_fields) found in response" >&2
			echo "Response content preview: $(echo "$cleaned_content" | head -c 200)..." >&2
			return 1
		fi
	else
...
@test "export individual conversations with markdown format" {
	...
	if echo "$content" | jq -e 'has("format")' >/dev/null 2>&1; then
		...
		if echo "$content" | jq -e '.format == "md" or .format == "markdown"' >/dev/null 2>&1; then
			...
			echo "Expected format 'md' or 'markdown', got '$format'" >&2
			return 1
		fi
	else

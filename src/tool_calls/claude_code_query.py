...
class ClaudeCodeQuery:
    ...
    def find_session_files(
        ...
        if project_root:
            ...
            project_sessions_dir = self.projects_dir / encoded_path

            if project_sessions_dir.exists():
                session_files.extend(project_sessions_dir.glob("*.jsonl"))

        # Also check global sessions directory
...
def handle_query_claude_conversations(
    ...
    try:
        ...
        # Query data - for Claude Code, use user home directory to find conversation files
        ...
        data = query_tool.query_conversations(
            conversation_project_root, limit=limit
        )
...
def handle_search_claude_conversations(
    ...
    try:
        ...
        results = query_tool.search_conversations(
            query, conversation_project_root, limit=limit
        )
        ...
        for result in results:
            ...
            for match in result["matches"]:
                ...
                if include_content:
                    match_data["full_content"] = match["message"].get(
                        "content", ""
                    )
        ...
        log_info(
            f"Found {len(formatted_results)} conversations matching '{query}'"
        )
        return AccessValidator.create_success_response(

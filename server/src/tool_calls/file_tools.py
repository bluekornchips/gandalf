...
from src.utils.performance import start_timer, log_operation_time
...
def validate_file_types(file_types: List[str]) -> tuple[bool, str]:
    """
    Validate file types array for security and format compliance.

    Args:
        file_types: List of file extensions to validate

    Returns:
        Tuple of (is_valid, error_message)
    """
    if len(file_types) > MAX_FILE_TYPES:
        return False, f"Too many file types specified (max {MAX_FILE_TYPES})"

    # Use security validator for consistent validation
    for file_type in file_types:
        is_valid, error_msg = SecurityValidator.validate_file_extension(file_type)
        if not is_valid:
            return False, f"Invalid file extension '{file_type}': {error_msg}"

        if len(file_type) > MAX_FILE_EXTENSION_LENGTH:
            return False, f"File extension too long: {file_type}"
...
def handle_list_project_files(
    ...
    try:
        ...
        # project_root
        valid, error_msg = SecurityValidator.validate_path(project_root, "project_root")
        if not valid:
            return SecurityValidator.create_error_response(error_msg)
        ...
        return SecurityValidator.create_error_response(f"Error listing files: {str(e)}")

...
from src.utils.performance import start_timer, get_duration
...
def _get_file_stats_fast(project_root: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if file_result.returncode == 0 and dir_result.returncode == 0:
            ...
            return {"total_files": 0, "total_directories": 0, "method": "find_failed"}

    except (subprocess.SubprocessError, subprocess.TimeoutExpired, OSError) as e:
        log_debug(f"Find command failed for {project_root}: {e}")
        return {"total_files": 0, "total_directories": 0, "method": "find_error"}


def _get_file_stats_with_cache_optimization(project_root: Path) -> Dict[str, Any]:
...
def _get_file_statistics(project_root: Path) -> Dict[str, Any]:
    ...
    if not project_root.exists():
        return {"total_files": 0, "total_directories": 0, "method": "path_not_found"}
...
def _create_basic_project_info(project_root: Path) -> Dict[str, Any]:
    ...
    sanitized_project_name = SecurityValidator.sanitize_project_name(raw_project_name)

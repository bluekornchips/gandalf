...
CONVERSATION_PATTERNS = WINDSURF_CONVERSATION_PATTERNS
STRONG_CONVERSATION_INDICATORS = WINDSURF_STRONG_CONVERSATION_INDICATORS
FALSE_POSITIVE_INDICATORS = WINDSURF_FALSE_POSITIVE_INDICATORS
CONTENT_KEYS = WINDSURF_CONTENT_KEYS
MESSAGE_INDICATORS = WINDSURF_MESSAGE_INDICATORS
...
class ConversationValidator:
    ...
    def is_valid_conversation(data: Any) -> bool:
        ...
        false_positive_count = sum(
            1
            for indicator in WINDSURF_FALSE_POSITIVE_INDICATORS
            if indicator in data_str
        )
    ...
    def _validate_dict_structure(data: Dict[str, Any]) -> bool:
        ...
        has_content = any(key in data for key in WINDSURF_CONTENT_KEYS)
        if not has_content:
            return False

        # Check if content is meaningful
        for key in WINDSURF_CONTENT_KEYS:
            if key in data:
                content = data[key]
                if (
        ...
        for item in data[:MAX_LIST_ITEMS_TO_CHECK]:
            if isinstance(item, dict):
                ...
                has_message_indicators = any(
                    indicator in item_str for indicator in WINDSURF_MESSAGE_INDICATORS
                )
                has_content_keys = any(key in item for key in WINDSURF_CONTENT_KEYS)
...
class WindsurfQuery:
    ...
    def _find_workspace_databases(self) -> List[Path]:
        ...
        # Search workspace storage (handle as list)

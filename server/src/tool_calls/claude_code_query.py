...
class ClaudeCodeQuery:
    ...
    def find_session_files(self, project_root: Optional[Path] = None) -> List[Path]:
        ...
        # Always search in all project directories since Claude Code stores conversations by project
        ...
        # If a specific project root is provided, filter to only that project's conversations
...
def handle_search_claude_conversations(
    arguments: Dict[str, Any], project_root: Path, **kwargs
) -> Dict[str, Any]:
    """Search Claude Code conversations for specific content."""
    try:
        ...
        data = query_tool.search_conversations(query, project_root, limit=limit)
        ...
        for result in data:
            ...
            for match in result["matches"]:
                ...
                if include_content:
                    match_data["full_content"] = match["message"].get("content", "")
        ...
        log_info(f"Found {len(formatted_results)} conversations matching '{query}'")
        return AccessValidator.create_success_response(
...
TOOL_SEARCH_CLAUDE_CONVERSATIONS = {
    "name": "search_claude_conversations",
    "description": "Search Claude Code conversation history for specific content or topics",
    "inputSchema": {
        "type": "object",
        "properties": {
            "query": {
    ...
    "annotations": {
        "title": "Search Claude Code Conversations",
        "readOnlyHint": True,
        "destructiveHint": False,
        "idempotentHint": True,
        "openWorldHint": False,
...
CLAUDE_CODE_QUERY_TOOL_HANDLERS = {
    ...
    "search_claude_conversations": handle_search_claude_conversations,
}

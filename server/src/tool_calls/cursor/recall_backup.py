...
def get_project_cache_hash(project_root: Path, context_keywords: list[str]) -> str:
    ...
    try:
        ...
        return hashlib.md5(
            f"{project_root}{time.time()}".encode(), usedforsecurity=False
        ).hexdigest()[:16]
...
def is_cache_valid(project_root: Path, context_keywords: list[str]) -> bool:
    ...
    try:
        ...
        return bool(project_hash == current_hash) if project_hash is not None else False
...
def score_recency(conversation: dict[str, Any]) -> float:
    ...
    try:
        ...
        if days_ago <= 1:
            return float(recency_thresholds.get("days_1", 0.8))
        elif days_ago <= 7:
            return float(recency_thresholds.get("days_7", 0.6))
        elif days_ago <= 30:
            return float(recency_thresholds.get("days_30", 0.4))
        elif days_ago <= 90:
...
def analyze_conversation_relevance(
    ...
    if not isinstance(keyword_matches, list):
        keyword_matches = []
    analysis["conversation_type"] = classify_conversation_type(
        conv_text, keyword_matches, []
    )
...
def handle_recall_cursor_conversations(  # noqa: C901
    ...
    try:
        ...
        final_conversations: list[dict[str, Any]] = relevant_conversations[:limit]
        ...
        conversations_list: list[dict[str, Any]] = result["conversations"]
        processing_stats: dict[str, Any] = result["processing_stats"]
        ...
        log_info(
            f"Recalled {len(conversations_list)} conversations "

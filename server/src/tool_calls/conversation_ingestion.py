"""
Conversation ingestion and analysis for Gandalf MCP server.
"""
...
from cursor_chat_query import CursorQuery
...
from config.constants.technology import TECHNOLOGY_KEYWORD_MAPPING
from config.weights import CONVERSATION_WEIGHTS
...
from src.utils.performance import start_timer, log_operation_time, get_duration
from src.utils.security import SecurityValidator

from src.core.conversation_storage import (
    generate_context_keywords,
    is_storage_valid as is_cache_valid,
    load_stored_conversations as load_cached_conversations,
    save_conversations_to_storage as save_conversations_to_cache,
)
...
# Define the cache TTL constant locally since it's used here
CONTEXT_CACHE_TTL_SECONDS = 300  # 5 minutes


def get_cache_directory() -> Path:
    """Get or create the cache directory for conversations in GANDALF_HOME."""
...
def get_cache_file_path() -> Path:
    """Get the full path to the conversation cache file in GANDALF_HOME."""
    return CONVERSATION_CACHE_FILE


def get_cache_metadata_path(project_root: Path) -> Path:
    """Get the full path to the cache metadata file in GANDALF_HOME."""
...
def is_cache_valid(project_root: Path, context_keywords: List[str]) -> bool:
    ...
    try:
        metadata_path = get_cache_metadata_path(project_root)
...
def save_conversations_to_cache(
    ...
    try:
        ...
        cache_size_mb = cache_file_path.stat().st_size / (1024 * 1024)
        log_info(f"Cached {len(conversations)} conversations ({cache_size_mb:.1f}MB)")
...
def extract_snippet(text: str, query: str) -> str:
    ...
    try:
        ...
        start = max(0, index - CONVERSATION_SNIPPET_CONTEXT_CHARS // 2)
            ...
            len(text), index + len(query) + CONVERSATION_SNIPPET_CONTEXT_CHARS // 2
        )
...
def handle_ingest_conversations(
    ...
    try:
        ...
        if (
            ...
            or days_lookback < 1
            or days_lookback > 60  # Max 60 days lookback, move this to a constant
        ):
            return SecurityValidator.create_error_response(
                f"days_lookback must be an integer between 1 and 60"
            )
        ...
        filtered_conversations.sort(
            key=lambda x: x["conversation"].get("lastUpdatedAt", 0), reverse=True
        )
        ...
        return SecurityValidator.create_success_response(json.dumps(result, indent=2))
...
def handle_fast_conversation_extraction(
    ...
    for workspace in data["workspaces"]:
        ...
        # Pre-build conversation ID lookup for efficiency
        prompt_counts = {}
        generation_counts = {}
        ...
        for prompt in workspace["prompts"]:
            ...
            if conv_id:
                prompt_counts[conv_id] = prompt_counts.get(conv_id, 0) + 1

        for generation in workspace["generations"]:
            conv_id = generation.get("conversationId", "")
            if conv_id:
                generation_counts[conv_id] = generation_counts.get(conv_id, 0) + 1
        ...
        for conversation in workspace["conversations"]:
            ...
            # Pre-computed conversation stats
            conv_id = conversation.get("composerId", "")
            prompt_count = prompt_counts.get(conv_id, 0)
            generation_count = generation_counts.get(conv_id, 0)

            # Skip conversations with very few exchanges (likely not useful)
            total_exchanges = min(prompt_count, generation_count)
            if total_exchanges == 0 and not conv_name:
                skipped_count += 1
                continue
    ...
    result = {
        ...
        "processing_stats": {
            ...
            "efficiency_percent": (
                round((processed_count - skipped_count) / processed_count * 100, 1)

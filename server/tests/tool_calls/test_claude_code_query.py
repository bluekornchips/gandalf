...
class TestClaudeCodeQueryHandlers:
    ...
    @patch("src.tool_calls.claude_code_query.ClaudeCodeQuery")
    def test_handle_search_claude_conversations_success(self, mock_query_class):
    ...
    @patch("src.tool_calls.claude_code_query.ClaudeCodeQuery")
    def test_handle_search_claude_conversations_with_content(self, mock_query_class):
        ...
        data = json.loads(result["content"][0]["text"])
        assert (
            data["results"][0]["matches"][0]["full_content"] == "full message content"
        )

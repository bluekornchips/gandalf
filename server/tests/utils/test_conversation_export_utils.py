...
class TestConversationExportMainFunction:
    """Test the main function behavior (CLI interface)."""
    ...
    @patch("src.utils.conversation_export.list_workspaces")
    @patch(
        "sys.argv",
        ["conversation_export.py", "output.json", "--list-workspaces"],
    )
    def test_main_list_workspaces(self, mock_list_workspaces):
        """Test main function with --list-workspaces flag."""
        mock_list_workspaces.return_value = ["workspace1", "workspace2"]

        # Import and run the main block
        import src.utils.conversation_export as ce_module

        # Mock sys.argv and argparse
        with patch("argparse.ArgumentParser.parse_args") as mock_parse_args:
            mock_args = Mock()
            mock_args.list_workspaces = True
            mock_args.silent = False
            mock_parse_args.return_value = mock_args

            # This would normally be in the if __name__ == "__main__" block
            # We'll test the logic directly
            if mock_args.list_workspaces:
                ce_module.list_workspaces(silent=mock_args.silent)

        mock_list_workspaces.assert_called_once_with(silent=False)

    @patch("src.utils.conversation_export.export_conversations_simple")
    @patch("sys.exit")
    def test_main_export_success(self, mock_exit, mock_export):
        """Test main function with successful export."""
        mock_export.return_value = True

        # Mock argparse
        with patch("argparse.ArgumentParser.parse_args") as mock_parse_args:
            mock_args = Mock()
            mock_args.list_workspaces = False
            ...
            success = ce_module.export_conversations_simple(
                output_path=mock_args.output_path,
                format_type=mock_args.format,
                silent=mock_args.silent,
            )
        ...
        mock_export.assert_called_once_with(
            output_path="output.json", format_type="json", silent=False
        )
        mock_exit.assert_called_once_with(0)

    @patch("src.utils.conversation_export.export_conversations_simple")
    @patch("sys.exit")
    def test_main_export_failure(self, mock_exit, mock_export):
        ...
        mock_export.assert_called_once_with(
            output_path="output.json", format_type="json", silent=True
        )

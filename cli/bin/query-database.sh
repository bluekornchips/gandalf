...
check_format_tool() {
	...
	json)
		if command -v jq >/dev/null 2>&1; then
			...
			echo "jq is required for JSON formatting but not installed" >&2
			return 1
		fi
		;;
	yaml)
		if command -v yq >/dev/null 2>&1; then
			...
			echo "yq is required for YAML formatting but not installed" >&2
			return 1
		fi
		;;
	*)
		echo "Invalid output format: $format. Supported formats: json, yaml" >&2
		return 1
		;;
	esac
}

...
@test "conversation analysis performs well with large conversation history" {
    # Test conversation analysis performance with streamlined tools
    local start_time end_time duration
    start_time=$(date +%s)
...
@test "memory usage stays reasonable during operations" {
    ...
    # Multiple conversation operations using streamlined aggregation tools
    for i in {1..3}; do
        execute_rpc "tools/call" '{"name": "recall_conversations", "arguments": {"fast_mode": true, "limit": 10}}' >/dev/null
    done

    # Search operations using aggregated search
...
@test "extreme weight configurations don't crash system" {
    # Skip if gandalf-weights.yaml doesn't exist (not all setups may have it)
    if [[ ! -f "$GANDALF_ROOT/spec/gandalf-weights.yaml" ]]; then
        skip "gandalf-weights.yaml not found - skipping extreme weights test"
    fi

    # Backup original weights
    local backup_file=$(mktemp)
    cp "$GANDALF_ROOT/spec/gandalf-weights.yaml" "$backup_file"

    # Use Sauron's extreme weights configuration - maximum power test
    cp "$GANDALF_ROOT/scripts/tests/fixtures/data/saurons_weights.yaml" "$GANDALF_ROOT/spec/gandalf-weights.yaml"

    run execute_rpc "tools/call" '{"name": "list_project_files", "arguments": {"use_relevance_scoring": true, "max_files": 5}}'
    local exit_code=$status

    # Restore original weights
    cp "$backup_file" "$GANDALF_ROOT/spec/gandalf-weights.yaml"
    rm -f "$backup_file"
...
@test "inverted priority configuration works correctly" {
    # Skip if gandalf-weights.yaml doesn't exist
    if [[ ! -f "$GANDALF_ROOT/spec/gandalf-weights.yaml" ]]; then
        skip "gandalf-weights.yaml not found - skipping inverted priorities test"
    fi
    ...
    cp "$GANDALF_ROOT/scripts/tests/fixtures/data/weights_inverted.yaml" "$GANDALF_ROOT/spec/gandalf-weights.yaml"
...
@test "context intelligence disabled mode works" {
    # Skip if gandalf-weights.yaml doesn't exist
    if [[ ! -f "$GANDALF_ROOT/spec/gandalf-weights.yaml" ]]; then
        skip "gandalf-weights.yaml not found - skipping disabled mode test"
    fi
    ...
    cp "$GANDALF_ROOT/scripts/tests/fixtures/data/shire_weights.yaml" "$GANDALF_ROOT/spec/gandalf-weights.yaml"

...
@test "Strategy 1: WORKSPACE_FOLDER_PATHS takes highest priority" {
    ...
    run bash -c "cd '$GANDALF_ROOT/server' && env WORKSPACE_FOLDER_PATHS='$workspace_project' PYTHONPATH=. '$python_cmd' src/main.py <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Strategy 1: Multiple workspace paths - uses first valid one" {
    ...
    local python_cmd=$(get_python_executable)
    run bash -c "cd '$GANDALF_ROOT/server' && env WORKSPACE_FOLDER_PATHS='$workspace1:$workspace2' PYTHONPATH=. '$python_cmd' src/main.py <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Strategy 2: Git root detection in workspace paths" {
    ...
    # Server should detect git root from the subdirectory
    local python_cmd=$(get_python_executable)
    run bash -c "cd '$git_subdir' && PYTHONPATH='$GANDALF_ROOT/server' '$python_cmd' '$GANDALF_ROOT/server/src/main.py' <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Strategy 2: Git detection fallback to current directory" {
    ...
    local python_cmd=$(get_python_executable)
    # Fixed: Direct server execution
    run bash -c "cd '$GANDALF_ROOT/server' && PYTHONPATH=. '$python_cmd' src/main.py --project-root '$git_project' <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Strategy 3: PWD environment variable fallback" {
    ...
    local python_cmd=$(get_python_executable)
    run bash -c "cd /tmp && env PWD='$pwd_project' PYTHONPATH='$GANDALF_ROOT/server' '$python_cmd' '$GANDALF_ROOT/server/src/main.py' <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Strategy 4: Current working directory final fallback" {
    ...
    local python_cmd=$(get_python_executable)
    # Fixed: Direct server execution
    run bash -c "cd '$GANDALF_ROOT/server' && env -u PWD -u WORKSPACE_FOLDER_PATHS PYTHONPATH=. '$python_cmd' src/main.py --project-root '$cwd_project' <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Real-world scenario: Cursor MCP server startup" {
    ...
    local python_cmd=$(get_python_executable)
    # Fixed: Direct server execution
    run bash -c "cd '$GANDALF_ROOT/server' && env WORKSPACE_FOLDER_PATHS='$cursor_workspace' PYTHONPATH=. '$python_cmd' src/main.py <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"list_project_files\", \"arguments\": {\"file_types\": [\".js\", \".md\"]}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Error handling: Invalid workspace paths" {
    local python_cmd=$(get_python_executable)
    # Fixed: Direct server execution
    run bash -c "cd '$GANDALF_ROOT/server' && env WORKSPACE_FOLDER_PATHS='/nonexistent/path:/another/invalid/path' PYTHONPATH=. '$python_cmd' src/main.py <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "Workspace detection logging verification" {
    ...
    local python_cmd=$(get_python_executable)
    # Fixed: Direct server execution
    run bash -c "cd '$GANDALF_ROOT/server' && env WORKSPACE_FOLDER_PATHS='$workspace_project' PYTHONPATH=. '$python_cmd' src/main.py <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}'"
    [ "$status" -eq 0 ]
...
@test "Strategy priority ordering verification" {
    ...
    # Setup git in workspace path
    cd "$workspace_path"
    git init >/dev/null 2>&1
    git config user.email "test@gandalf.test"
    git config user.name "Gandalf Test"
    echo "# Priority Test" >README.md

    local python_cmd=$(get_python_executable)
    # Fixed: Direct server execution
    run bash -c "cd '$GANDALF_ROOT/server' && env WORKSPACE_FOLDER_PATHS='$workspace_path' PWD='$pwd_path' PYTHONPATH=. '$python_cmd' src/main.py <<< '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' 2>/dev/null"
    [ "$status" -eq 0 ]
    ...
    # Compare resolved paths since server resolves symlinks
    local expected_workspace_resolved
    expected_workspace_resolved=$(cd "$workspace_path" && pwd -P)
    [[ "$detected_root" == "$expected_workspace_resolved" ]]
}

...
@test "server handles non-git directories gracefully" {
    ...
    run bash -c "cd '$GANDALF_ROOT/server' && echo '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' | PYTHONPATH=. '$python_cmd' src/main.py --project-root '$non_git_project' 2>/dev/null"
    [ "$status" -eq 0 ]
...
@test "server handles empty directories gracefully" {
    ...
    local python_cmd=$(get_python_executable)
    run bash -c "cd '$GANDALF_ROOT/server' && echo '{\"jsonrpc\": \"2.0\", \"method\": \"tools/call\", \"id\": 1, \"params\": {\"name\": \"get_project_info\", \"arguments\": {}}}' | PYTHONPATH=. '$python_cmd' src/main.py --project-root '$empty_project' 2>/dev/null"
    [ "$status" -eq 0 ]
